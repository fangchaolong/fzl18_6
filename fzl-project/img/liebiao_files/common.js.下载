...
//			document.getElementById('tell_me_form').style.display = 'block';
		...
		var cart_url = domain_url+'flow.php?step=cart';
	        ...
	        if(window.location.port){  //cjc+ 如果有端口号，跳转地址加上url端口号
	            var domain_url_length = domain_url.length - 1;
	            var reg =eval("/(.{"+ domain_url_length +"})(.*)/");
	            var newdomain_url = domain_url.replace(reg, "$1:"+window.location.port+"$2");
	            var cart_url = newdomain_url+'flow.php?step=cart';
	        }
...
function opencartDiv(price, name, pic, goods_brief, goods_id, total, number) {
	...
	// 生成层内内容
	html = '<div class=cardivfloat><span class=cartdivfloattitle>商品已成功添加到购物车！</span><a href=\'javascript:cancel_div()\' style="float:right;padding:0 26px 0 0;background:url(themes/juhuimai/images/ico_closebig1.gif) right center no-repeat;cursor:pointer;color:#ffffff;font-size:12px;" >关闭</a></div><div class="cartpopDiv"><div class="toptitle"><a href="goods.php?id=' + goods_id + '" class="pic"><img src=' + pic + ' width="98" height="98" style="border:#dddddd 1px solid; display:block;"/></a><p><font style="font-weight:bold">' + name + '</font>  <br>' + goods_brief + '<br>购买价格：<font style="color:#cc0000">' + price + '</font><br></p></div>';
	...
	html += "<div class=cartbntfloat ><a href='flow.php'><img src='themes/juhuimai/images/jsico1.gif'></a><a href=\'javascript:cancel_div()\'><img src='themes/juhuimai/images/goon_ico1.gif'></a></div>";
	html += '</div></div>';
